...
public class UserController {
	 ...
	 */
	@PostMapping("/regist")
	public String doRegist(@ModelAttribute User user, @RequestParam("file") MultipartFile file, Model model)
			throws IllegalStateException, IOException {

		// 먼저 파일이 존재하는지 검사
		if (file != null && file.getSize() > 0) {
			// 파일을 저장할 위치 지정
			Resource res = resLoader.getResource("classpath:/static/resources/img/");
