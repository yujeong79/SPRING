...
public class BoardController {
	...
	public BoardController() {
		boards.add(new Board(nxtId++, "알고리즘 스터디 모집해요", "SW역량테스트 A형 준비하고 있는데 같이 할 사람 구해요!", new Date(), "익명"));
		boards.add(new Board(nxtId++, "Sprin Boot 스터디 모집", "Spring Boot 공부하고 있는데 같이 할 사람 구해요!", new Date(), "익명"));
		boards.add(new Board(nxtId++, "SSAFY 1학기를 진행하면서", "1학기 고생 많았어요! 같이 회고해요!", new Date(), "익명"));
	}
    ...
    public String create(@ModelAttribute Board board, HttpSession session) {
    	...
    	board.setAuthor((String)session.getAttribute("id"));
        boards.add(board);
        return "redirect:/board/list";
    }
    /*
     ...
     * 게시글 목록 /board/list로 리다이렉트
     * 
     * 심화 Q. 게시글 수정 시 작성자를 확인하도록 로직을 추가해보세요.
     */
    ...
    public String update(@PathVariable int id, @ModelAttribute Board updatedBoard, HttpSession session) {
        ...
        for (Board board : boards) {
            if (board.getId() == id && board.getAuthor().equals(author)) {
	 ...
	 * 
	 * 심화 Q. 게시글 삭제 시 작성자를 확인하도록 로직을 추가해보세요.
	 */
